  ...
  return (
    <section className="py-36 md:py-44 bg-texture bg-wood">
      <div className="mx-auto max-w-7xl px-6 sm:px-8 lg:px-10">
        ...
        <StaggerContainer className="mt-16 md:mt-20 grid grid-cols-1 gap-8 md:grid-cols-3">
          {testimonials.map((t) => (
            <StaggerItem key={t.name}>
              <ArchCard className="relative h-full">
                <ArchCardContent className="pt-10 pb-8">
                  <Quote className="mb-5 h-8 w-8 text-[--accent-gold] opacity-25" />
                  <p className="font-quote text-base leading-relaxed italic text-[--text-secondary]">
                    &ldquo;{t.quote}&rdquo;
                  </p>
                  <div className="mt-8 border-t border-[--border-hairline] pt-5">
                    <p className="text-base font-medium text-[--text-primary]">{t.name}</p>
